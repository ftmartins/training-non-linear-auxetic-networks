...
def load_training_result(task_seed, realization_seed, results_dir=None):
    ...
    print(f"History loaded with {len(history.get('loss', []))} steps")
    print(f"Loading network from: {result_path}")
    # Load network
    with open(result_path / "final_network.pkl", "rb") as f:
        network = pickle.load(f)

    print(f"Network loaded with {len(network.get('edges', []))} edges and {len(network.get('positions', []))} nodes")
    # Load config
