...
def compute_unified_mode_data(
    ...
    for ti, t in enumerate(t_indices):
        ...
        s_par, s_perp, s_eq, s_tot = _susceptibility_components_per_edge(
            pos_t, result["network"], result["boundary_dict"]
        )
        norm_s = float(np.linalg.norm(s_tot))
